...
export class Chunk {
	...
	static add_chunk(chunk: Chunk) {
		...
		Server.runCommand("scoreboard players set \"$db_chunk(" + textToHex(JSON.stringify(chunk)) + ")\" db_chunk 1");
	...
	static remove_chunk(chunk: Chunk) {
		...
		//Server.runCommand("scoreboard players reset \"$db_chunk(" + textToHex(JSON.stringify(chunk)) + ")\" db_chunk");
