...
export class Faction {
	...
	static async initDB_faction() {
		if (db_faction.size === 0) {
			...
			await Server.runCommand(`scoreboard objectives add ${objectiveName} dummy`);
	...
	isAtLeastRank(member: faction_member | faction_member['name'], fRank: (typeof factionRank[keyof typeof factionRank])): boolean {
	...
	remove_to_update_faction() {
		...
		Server.runCommand("scoreboard players reset \"$db_faction(" + textToHex(this.jsonString(this)) + ")\" db_faction");
	}

	add_to_update_faction() {
		if (this === undefined) return;
		Server.runCommand("scoreboard players set \"$db_faction(" + textToHex(this.jsonString(this)) + ")\" db_faction 1");
	}
	...
	static add_faction(faction: Faction | undefined) {
		...
		db_faction.set(faction.name, faction);
		Server.runCommand("scoreboard players set \"$db_faction(" + textToHex(faction.jsonString(faction)) + ")\" db_faction 1");
	}

	static remove_faction(faction: Faction | undefined) {
		if (faction === undefined) return;
		Server.runCommand("scoreboard players reset \"$db_faction(" + textToHex(faction.jsonString(faction)) + ")\" db_faction");
