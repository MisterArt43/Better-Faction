...
export class Delay {
	...
	static add_delay(delay: Delay) {
		...
		Server.runCommand("scoreboard players set \"$db_delay(" + textToHex(JSON.stringify(delay)) + ")\" db_delay 1");
	}
	...
	static remove_delay(delay: Delay) {
		...
		Server.runCommand("scoreboard players reset \"$db_delay(" + textToHex(JSON.stringify(delay)) + ")\" db_delay");
		db_delay.delete(delay.name);
	}

	add_to_update_delay() {
		Server.runCommand("scoreboard players set \"$db_delay(" + textToHex(JSON.stringify(this)) + ")\" db_delay 1");
	}

	remove_to_update_delay() {
		Server.runCommand("scoreboard players reset \"$db_delay(" + textToHex(JSON.stringify(this)) + ")\" db_delay");
	}
