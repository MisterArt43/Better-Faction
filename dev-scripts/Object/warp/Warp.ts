...
export class Warp {
	...
	static add_warp(warp: Warp) {
		...
		Server.runCommand("scoreboard players set \"$db_warp(" + textToHex(JSON.stringify(warp)) + ")\" db_warp 1");
	}
	...
	static remove_warp(warp: Warp) {
		...
		Server.runCommand("scoreboard players reset \"$db_warp(" + textToHex(JSON.stringify(warp)) + ")\" db_warp");
		db_warp.delete(warp.name);
	}

	add_to_update_warp() {
		Server.runCommand("scoreboard players set \"$db_warp(" + textToHex(JSON.stringify(this)) + ")\" db_warp 1");
	}

	remove_to_update_warp() {
		Server.runCommand("scoreboard players reset \"$db_warp(" + textToHex(JSON.stringify(this)) + ")\" db_warp");
	}
