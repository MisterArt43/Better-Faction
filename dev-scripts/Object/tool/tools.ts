...
export function tellraw(selector : tellrawSelector, text : string) {
	if (selector instanceof Player) {
		...
		Server.runCommand(`tellraw ${selector} {"rawtext":[{"text":"§r${text.replace(/"/g, "\'")}"}]}`);
	else
		Server.runCommand(`tellraw "${selector}" {"rawtext":[{"text":"§r${text.replace(/"/g, "\'")}"}]}`);
}
...
export function log(text : number | string) {
	...
	//Server.runCommand(`tellraw @a[tag=log] {"rawtext":[{"text":"§7{log} §r${formatedText}"}]}`)
	if (isLoaded) {
...
export function getMap(regexObj: RegExp): string[] {
	try {
		...
		try {
			...
			Server.runCommand(`say getBinary error: ${e}`);
		}
...
export async function runCommandDim(command: string, dimension: Dimension['id']) {
	try {
		return { error: false, ...await world.getDimension(dimension).runCommand(command)};//to fix at the next update
	} catch (error) {
...
export async function tpsound(player: Player) {
	await sleep(2);
	await player.runCommand(`playsound mob.shulker.teleport`);
}