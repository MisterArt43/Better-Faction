...
export class DB_Map {
	...
	static async initDB_map() : Promise<DB_Map> {
		if (db_map === undefined) {
			await Server.runCommand("scoreboard objectives add database dummy");
			try {
				...
				if (db_map == undefined) {
					...
					await Server.runCommand("scoreboard players reset \"$db_map(" + textToHex(JSON.stringify(db_map)) + ")\" database");
					update_db_map(new DB_Map());
					await Server.runCommand("scoreboard players set \"$db_map(" + textToHex(JSON.stringify(db_map)) + ")\" database 1");
				}
			}
	...
	static async UpdateDB() {
		if (db_map !== undefined && isLoaded === false) {
			...
			//await Server.runCommand("scoreboard players set \"$db_map(" + textToHex(JSON.stringify(new_obj)) + ")\" database 1");
