...
system.runInterval(async () => {
	if (isLoaded && curTick > START_TICK) {
		...
		if (players.length !== 0 && DB.db_map != undefined) {
			...
			if (curTick % (DB.db_map.refreshTick + 1) === 0) Server.runCommand("title @a[tag=debug] actionbar " + Math.round(curTick / 20) + " s, script is now running.\n§7> RefreshTime : §s" + loadDatabase.refreshTime);
				...
				if (refreshCalc) {
					...
					if (p.hasTag("ban") && player !== undefined) {
						if (player.isunban || (isAdmin)) {
							...
							Server.runCommand(`kick "${p.name}" §c•> You are ban`);
						}
					}
		...
		if (curTick % 10 === 0) {
			Server.runCommand("title @a[tag=log] actionbar " + loadDatabase.player + "\n" + loadDatabase.faction + "\n" + loadDatabase.chunk + "\n" + loadDatabase.delay);
		}
	}
	curTick++;
}, 1)