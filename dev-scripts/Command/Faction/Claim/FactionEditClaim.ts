...
function editUI(player: Player, ply: Ply, fac: Faction, chunk?: Chunk) {
	...
	.title("edit claim")
	.dropdown("claim group", listGroup, {defaultValueIndex: listGroup.indexOf(chunk.group)})
	...
	for(const rank of Object.keys(factionRank)) {
		...
		form.dropdown("\nCan Break permission for " + rank, ["true", "false"], {defaultValueIndex: chunk.rankPermission.find(p => p.rank === Object.keys(factionRank).indexOf(rank))?.permission.canBreak ? 0 : 1});
		form.dropdown("Can Place permission for " + rank, ["true", "false"], {defaultValueIndex: chunk.rankPermission.find(p => p.rank === Object.keys(factionRank).indexOf(rank))?.permission.canPlace ? 0 : 1});
		form.dropdown("Can Interact permission for " + rank, ["true", "false"], {defaultValueIndex: chunk.rankPermission.find(p => p.rank === Object.keys(factionRank).indexOf(rank))?.permission.canInteract ? 0 : 1});
	}

	form.dropdown("\n\n stranger player permission :\n Can Break", ["true", "false"], {defaultValueIndex: chunk.defaultPermission.canBreak ? 0 : 1});
	form.dropdown("Can Place", ["true", "false"], {defaultValueIndex: chunk.defaultPermission.canPlace ? 0 : 1});
	form.dropdown("Can Interact", ["true", "false"], {defaultValueIndex: chunk.defaultPermission.canInteract ? 0 : 1});
