...
import { concatenateArgs, MinecraftDimensionTypes } from "../../Object/tool/tools";
...
function addWarpUI(player: Player, ply: Ply) {
	...
	new BFModalFormData()
		...
		.textField("X", "coordinate x", {defaultValue:player.location.x.toString()})
		.textField("Y", "coordinate y", {defaultValue:player.location.y.toString()})
		.textField("Z", "coordinate z", {defaultValue:player.location.z.toString()})
		.dropdown("Dimension", dimensionTab, {defaultValueIndex: dimensionTab.indexOf(player.dimension.id)})
		.toggle("is Open", {defaultValue: true})
		.textField("delay before a new tp to this warp", '5', {defaultValue: "5"})
