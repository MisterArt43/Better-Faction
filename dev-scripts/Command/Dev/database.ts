    ...
    switch (args[2]) {
        ...
        case "chunk" : {
            ...
            if (DB.db_chunk.size - 1 >= i) {
                log(colorizeJSON(JSON.stringify(Array.from(DB.db_chunk.values())[i], null, 8)));
                i++;
            }
...
function resetDB(args: string[], player: Player, ply: Ply) {
    ...
    .then(async res => {
        ...
        Server.runCommand("scoreboard objectives remove database")
        Server.runCommand("scoreboard objectives remove db_player")
        Server.runCommand("scoreboard objectives remove db_faction")
        Server.runCommand("scoreboard objectives remove db_display")
        Server.runCommand("scoreboard objectives remove db_delay")
        Server.runCommand("scoreboard objectives remove db_admin")
        Server.runCommand("scoreboard objectives remove db_warp")
        Server.runCommand("scoreboard objectives remove db_chunk")
