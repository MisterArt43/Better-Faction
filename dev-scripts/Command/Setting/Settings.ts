...
function set_command_prefix_ui(args: string[], player: Player, ply: Ply) {
	...
	form.show(player).then(async res => {
		...
		await Server.runCommand("scoreboard players reset \"$db_map(" + textToHex(JSON.stringify(DB.db_map)) + ")\" database");
		DB.db_map.prefix = res.formValues![0] as string;
		await Server.runCommand("scoreboard players set \"$db_map(" + textToHex(JSON.stringify(DB.db_map)) + ")\" database 1");
			...
			if (res.formValues![1]) {
				...
				system.run(async () => {
					...
					for (const player of DB.db_player.values()) {
						...
						if (index % 10 === 0) {
							await Server.runCommand(`title @a[tag=log] actionbar §eEditing Player Database Progression : §a${Math.floor(index / DB.db_player.size * 100)}%`);
							await sleep(2);
						}
						index++;
					}
