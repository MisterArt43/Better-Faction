...
export class Faction {
	...
	remove_to_update_faction() {
		...
		Server.runCommandAsync("scoreboard players reset \"$db_faction(" + textToHex(this.jsonString(this)) + ")\" db_faction");
	}

	add_to_update_faction() {
		if (this === undefined) return;
		Server.runCommandAsync("scoreboard players set \"$db_faction(" + textToHex(this.jsonString(this)) + ")\" db_faction 1");
	}
	...
	static add_faction(faction: Faction | undefined) {
		...
		db_faction.set(faction.name, faction);
		Server.runCommandAsync("scoreboard players set \"$db_faction(" + textToHex(faction.jsonString(faction)) + ")\" db_faction 1");
	}

	static remove_faction(faction: Faction | undefined) {
		if (faction === undefined) return;
		Server.runCommandAsync("scoreboard players reset \"$db_faction(" + textToHex(faction.jsonString(faction)) + ")\" db_faction");
