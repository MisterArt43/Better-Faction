...
function setPower(targetPly: Ply, player: Player, ply: Ply) {
    new BFModalFormData()
    .title("Set power")
    .textField("Power", "0", targetPly.power.toString())
    .submitButton("§cSet")
    .show(player).then(res => {
...
function setHomeLimit(targetPly: Ply, player: Player, ply: Ply) {
    ...
    .textField("Limit", "0", targetPly.homeLimit.toString())
    .submitButton("§cSet")
    .show(player).then(res => {
...
function setChat(targetPly: Ply, player: Player, ply: Ply) {
    new BFModalFormData()
    .title("Set chat")
    .dropdown("Chat", ["all", "faction", "ally"], targetPly.chat === "all" ? 0 : 1)
    .submitButton("§cSet")
    .show(player).then(res => {
...
function setPermission(targetPly: Ply, player: Player, ply: Ply) {
    ...
    .dropdown("Permission", permission, permission.indexOf(Object.keys(cmd_permission).find((key) => cmd_permission[key as keyof typeof cmd_permission] === targetPly.permission) as keyof typeof cmd_permission))
    .submitButton("§cSet")
    .show(player).then(res => {
...
function setModule(targetPly: Ply, player: Player, ply: Ply) {
    ...
    for (const mod of module) {
        form.toggle(mod, targetPly.cmd_module.includes(cmd_module[mod]) || targetPly.cmd_module.includes(cmd_module.all))
    }
